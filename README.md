...
Yes, first you need a PC to host your gaming session. Next, pick a host and client program.
...
- For me, I use **Apollo** and **Artemis** — they have better touch support for tablets, and allow a virtual screen so I can turn off my real monitor. The setup is similar to Sunshine.
...
## III. Port Forwarding (Direct Connection)
...
## IV. VPN / Tunnel (The Easiest & Recommended Method)
...
3. Note the Tailscale IP of your Host PC (e.g., `100.64.0.3`).
...
3. Add the Tailscale IP to `app_custom_address`:
   ```json
   "app_custom_address": "100.64.0.3"
   ```
4. Restart Parsec. This fixes the **6023 error**.

#### Configuring Moonlight for Tailscale:
- Simply "Add PC" manually in the Moonlight app using the Host's Tailscale IP.

---

## 🚀 V. Advanced: Using a VPS as a Relay (Ultra-Low Latency)
...
Replace `100.64.0.3` with your Host PC's Tailscale IP.
```bash
...
for port in 47984 47989 47990 48010; do
  sudo iptables -t nat -A PREROUTING -p tcp --dport $port -j DNAT --to-destination 100.64.0.3:$port
done
# UDP
for port in 47998 47999 48000 48002 48010; do
  sudo iptables -t nat -A PREROUTING -p udp --dport $port -j DNAT --to-destination 100.64.0.3:$port
done
```
